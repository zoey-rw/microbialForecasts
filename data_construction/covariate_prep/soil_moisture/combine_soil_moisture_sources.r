...
all_sources_merged <- merge(SMV_neon_merged, smos, by = c("siteID", "month"), all.x=T)
  ...
  soil.moisture.out <- all_sources_merged %>% mutate(moisture_out = ifelse(!is.na(neon_mean), neon_mean,
                                                     ...
                                                     source = ifelse(!is.na(neon_mean), "NEON",
                                                                     ifelse(!is.na(SCAN_est), "SCAN",
                                                                            ifelse(!is.na(SMAP_est), "SMAP",
                                                                                   "SMOS"))),
                                                     moisture_sd_out = ifelse(!is.na(neon_mean), neon_sd,
                                                                          ifelse(!is.na(SCAN_est), SCAN_site_sd,
                                                                                 ifelse(!is.na(SMAP_est), SMAP_site_sd,
                                                                                        SMOS_site_sd))))
  # Error bars = 1 standard deviation
  soil.moisture.out$hi <- soil.moisture.out$moisture_out + soil.moisture.out$moisture_sd_out
  soil.moisture.out$low <- soil.moisture.out$moisture_out - soil.moisture.out$moisture_sd_out
  
  
  # Mean-center and scale values for model
  sm.scale <- scale(soil.moisture.out$moisture_out)
  scale_factor <- attr(sm.scale, "scaled:scale")
  soil.moisture.out$moisture <- sm.scale
  # SD transformed by same scale factor 
  soil.moisture.out$moisture_sd <- soil.moisture.out$moisture_sd_out/scale_factor
  ### 
# Plots look alright: 
...
# As expected, worse data sources have larger uncertainties.
  ...
  p <- ggplot(soil.moisture.out, aes(x = date, y = moisture_out, color = source)) + 
  	...
  	geom_errorbar(aes(ymin=low,ymax=hi),alpha=0.3) + theme_minimal() + theme (legend.position = c(0.83, 0.04)) +
  ...
  fit2 <- lm(SMAP_est ~ neon_mean, soil.moisture.out)
  fit3 <- lm(SMOS_est ~ neon_mean, soil.moisture.out)
  p1 <- ggplot(fit1$model, aes_string(x = names(fit1$model)[2], y = names(fit1$model)[1])) + 
  	geom_point() +
  	stat_smooth(method = "lm", col = "red") +
  	labs(title = "Calibration fits", subtitle = paste("Adj R2 = ",signif(summary(fit1)$adj.r.squared, 4))) + 
  	xlab("Observed values from NEON") + ylab("Predicted values from SCAN") + 
  	theme_minimal() + theme (text = element_text(size = 12)) 
  p2 <- ggplot(fit2$model, aes_string(x = names(fit2$model)[2], y = names(fit2$model)[1])) + 
  	geom_point() +
  	stat_smooth(method = "lm", col = "red") +
  	labs(title = "Calibration fits", subtitle = paste("Adj R2 = ",signif(summary(fit2)$adj.r.squared, 4))) + 
  	xlab("Observed values from NEON") + ylab("Predicted values from SMAP") + 
  	theme_minimal() + theme (text = element_text(size = 12)) 
  p3 <- ggplot(fit3$model, aes_string(x = names(fit3$model)[2], y = names(fit3$model)[1])) + 
  	geom_point() +
  	stat_smooth(method = "lm", col = "red") +
  	labs(title = "Calibration fits", subtitle = paste("Adj R2 = ",signif(summary(fit3)$adj.r.squared, 4))) + 
  	xlab("Observed values from NEON") + ylab("Predicted values from SMOS") + 
  	theme_minimal() + theme (text = element_text(size = 12)) 
  p1
  p2
  p3
  ...
  library(ggpubr)
  ggarrange(p, 
  					ggarrange(p1, p2, p3,ncol=1, labels=c("B","C","D")), ncol = 2, widths = c(2,1), labels = c("A"))
