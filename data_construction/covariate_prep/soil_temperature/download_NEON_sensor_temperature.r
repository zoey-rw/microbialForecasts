...
library(data.table)
# neon_read_custom() and its helpers now come from the microbialForecast package.
...
# NEON soil temperature data, 30 min averages, from 3 shallowest depths.
...
df <- neon_read_custom(product = "DP1.00041.001", dir = store_dir)
dt <- data.table(df)
...
# 
# # read it in one at a time???
# # Now trying to download NEON soil temperature data, 30 min averages, from 3 shallowest depths.
# avail = Z10::dp.avail("DP1.00094.001")
# all_sites <- unlist(unique(avail$site))
# 
# 
# all_sites <- neon_sites()$siteCode
# site_df <- list()
# for (s in 1:length(all_sites)){
#   print(all_sites[s])
#   df <- neon_read_custom(product = "DP1.00041.001",  site = all_sites[s], dir = store_dir, altrep=T)
#   dt <- data.table(df)
#   dt[, `:=` (sensorID = paste0(siteID, "_", horizontalPosition, ".", verticalPosition),
#              month = substr(startDateTime, 1, 7),
#              depth = substr(verticalPosition, 3, 3),
