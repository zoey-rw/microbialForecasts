...
all_horizon_depths <- bd_ps %>% select(c(siteID,plotID,bulkDensTopDepth,bulkDensBottomDepth,horizon))
...
horizon_depths <- all_horizon_depths[all_horizon_depths$bulkDensTopDepth < 30,] %>% 
  ...
  slice(which.min(bulkDensTopDepth)) %>% filter(siteID %in% has_both)
...
expanded <- horizon_depths %>%
  ...
  unnest(data) %>% rename(depth = "data") %>% ungroup()
...
cutpoints <- cutpointr(expanded, depth, horizon, siteID,
                       ...
                       metric = accuracy)
key1 <- cutpoints %>% rename(siteID = "subgroup", bulkDensBottomDepth = "optimal_cutpoint") %>% mutate(bulkDensTopDepth = 0, horizon = "O") %>% select(siteID,bulkDensTopDepth, bulkDensBottomDepth, horizon)
key2 <- cutpoints %>% rename(siteID = "subgroup", bulkDensTopDepth = "optimal_cutpoint") %>% mutate(bulkDensBottomDepth = 30, horizon = "M") %>% select(siteID,bulkDensTopDepth, bulkDensBottomDepth, horizon)

# Keep only the shallowest mineral layer, which we'll use to determine a good cutpoint between organic and mineral
horizon_depth_key <- all_horizon_depths[all_horizon_depths$bulkDensTopDepth < 30,] %>% 
  group_by(siteID, horizon) %>% 
  slice(which.min(bulkDensTopDepth)) %>% filter(!siteID %in% has_both) %>% mutate(bulkDensBottomDepth = 30)
horizon_depth_key <- bind_rows(key1, key2, horizon_depth_key)

# Read in soil moisture sensor locations
sensors <- read.csv("/projectnb2/talbot-lab-data/zrwerbin/temporal_forecast/data/raw/NEONSoilMoist_daily/rawMoistStacked.rds/stackedFiles/sensor_positions_00094.csv")
...
sensors <- sensors %>% select(name, referenceName, siteID, sensorID, sensorDepth) %>% filter(sensorDepth < 25)
# Match to horizons
