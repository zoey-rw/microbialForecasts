---
title: "Particle Filter exercise - Zoey Werbin"
...
```{r, echo=FALSE}
# hold parameters constant as the mean of all values
constant.params <- list()
constant.params$falloc  = cbind(rep(mean(params$falloc[,1]), 400), # needs to have the same dimensions as the ensembles
                                rep(mean(params$falloc[,2]), 400),
                                rep(mean(params$falloc[,3]), 400))
constant.params$SLA     = mean(params$SLA)
constant.params$alpha   = mean(params$alpha)
constant.params$Q10     = mean(params$Q10)
constant.params$Rbasal  = mean(params$Rbasal)
constant.params$litter  = mean(params$litter)
constant.params$CWD     = mean(params$CWD)
constant.params$tau.leaf  = mean(params$tau.leaf)
constant.params$tau.stem  = mean(params$tau.stem)
constant.params$tau.soil  = mean(params$tau.soil)
...
for(t in 1:nt){
  ...
  ## forward step
  output.constant[t,,]=SSEM(X,constant.params,inputs[t,])
  X=output.constant[t,,1:3]
  ...
  if(t%%(48*8) == 0){            ## if at data frequence (remainder == 0)
    ...
    if(!is.na(LAIr[sample])){    ## if observation is present
      ...
      ## calulate Likelihood (weights)
      Lm = apply(output.constant[t+1-(48*8):1, ,4],2,mean)    ## average model LAI over obs period
      wt = dnorm(LAIr[sample],Lm,LAIr.sd[sample])    ## calculate likelihood (weight)
...
##### Plot resampling PF with and without parameter uncertainty #####

## Extract and summarize LAI (pr = PF, resampling)
LAIpr.constant = t(apply(output.constant[,,4],2,tapply,window,mean))         ## summarize PF LAI at measurment frequency
LAIpr.ci.constant  = apply(LAIpr.constant,2,quantile,c(0.025,0.5,0.975))     ## calculate median and CI
 
## plot time-series
plot(Mtime[Msel],LAIm.ci[2,],ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),
     type='n',ylab="LAI",xlab="Time")
ciEnvelope(Mtime[Msel],LAIpr.ci.constant[1,],LAIpr.ci.constant[3,],col=col.pf[2])
ciEnvelope(Mtime[Msel],LAIpr.ci[1,],LAIpr.ci[3,],col=col.pf[3])
points(Mtime,LAIr)    
for(i in 1:length(LAIr)){
  if(!is.na(QC[i])){
...
legend("topleft",legend=c("re-sampling PF without param uncert", "re-sampling PH"),col=col.pf[2:3],lwd=5)
```
