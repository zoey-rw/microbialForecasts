...
ncone.orig <- b$c00       	## number of cones in 2000
cones.orig <- (ncone > 0) 	## whether cones are present – this indicates whether tree is mature
...
# plot showing maturation (dia vs cones)
plot(dia,cones)                    ## raw data scatter plot
lines(xseq,pnorm(xseq,param[1],param[2]),type='l') # great! the line shows up.
...
likBin <- function(param){
    theta <- pnorm(dia, param[[1]], param[[2]]) # process model
    prob.cone <- dbinom(cones, prob = theta, size = 1, log=T)  # data model
    -sum(prob.cone)
}

likBin(param) # doesn't crash, that's good
nlm(likBin, param) # ignore NAs. estimates are ~22 and ~7
```
...
param = c(.05,20,8) # initial conditions set based on parameter estimates from previous analysis
...
```{r}
param  <- c(0.05,20,8)        ## initial guess (updated based on the first two models)
...
```{r}
theta <- pnorm(dseq,b0,b1) 	## maturation probit - theta(x)
g <- a0*dseq^2

plot(x = dia, y = ncone, main = "fecundity - dia vs ncone")
lines(dseq,g,col=3,lwd=3)  ## add fecundity relationship
lines(dseq, theta*g,lwd=3, col = "red") ## fecundity * theta

plot(x = dia, y = cones,  main = "maturation - dia vs cone")
lines(dseq, theta, type='l') # add theta curve
...
out.table <- cbind(a0, b0, b1, lnL, n)
rownames(out.table) <- c("full dataset", "ambient", "elevated CO2")
print(out.table)
```
...
```{r}
theta.full <- pnorm(dseq,b0[1],b1[1]) 	## maturation probit - theta(x)
g.full <- a0[1]*dseq^2
theta.amb <- pnorm(dseq,b0[2],b1[2]) 	## maturation probit - theta(x)
g.amb <- a0[2]*dseq^2
theta.co2 <- pnorm(dseq,b0[3],b1[3]) 	## maturation probit - theta(x)
g.co2 <- a0[3]*dseq^2

plot(x = dia, y = ncone, main = "fecundity - dia vs ncone")
lines(dseq, theta.full*g.full,lwd=3, col = "black") ## full model
lines(dseq, theta.amb*g.amb,lwd=3, col = "red") ## ambient-only
lines(dseq, theta.co2*g.co2,lwd=3, col = "blue") ## elevated-only

plot(x = dia, y = cones,  main = "maturation - dia vs cone")
lines(dseq, theta.full, type='l',lwd=3, col = "black") # add theta curve, full model
lines(dseq, theta.amb, type='l',lwd=3, col = "red") # add theta curve, ambient-only
lines(dseq, theta.co2, type='l',lwd=3, col = "blue") # add theta curve, elevated-only

```

### 1. Does elevated CO2 have an effect on fecundity? (Provide explanation for your answer).

Yes, CO2 has an effect on fecundity, visible in the first plot (above). The best-fit line for the elevated-CO2 model is higher and shifted to the left, which indicates that the trees in CO2 treatments produce more cones.  
...
The maturation rate is higher for CO2 treatment plots, indicated (in the second plot) by the blue best-fit line with the steeper slope and lower tree diameter midpoint. Trees exposed to elevated CO2 mature at smaller diameters. Even when accounting for the higher maturation rate, the increased fecundity can also be explained by a higher number of cones for a given diameter - evident in the trend of higher values for g(x) under elevated CO2 conditions. The best-fit lines for the full dataset are about midway between ambient and CO2 conditions, which is expected.
