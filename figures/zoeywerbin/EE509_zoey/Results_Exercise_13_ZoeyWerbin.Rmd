...
- The detrended data mostly fits the assumptions of second order stationarity. The residual mean is roughly zero, with a single point that skews the mean and makes the variance appear heteroskedastic. Because this is only driven by one point, I would say that it is close enough to the assumptions that we can use it for fitting our model.

3.	Does the first-difference time series meet the assumptions of second order stationarity?  Why or why not?

- Again, these residuals would meet the stationarity assumptions if one point was removed, so I would consider this to meet the stationarity assumptions.
...
Taking the first difference doesn't reduce the autocorrelation substantially, so we will fit the model using the detrended data (rx). The order for d should be 0 since we are not differencing the data. Based on the acf plot for the detrended data, we see that the lag is high at lag = 0, so the lag will likely be higher than 0. Since p=1 is a random walk (lag 1), we can try fitting the model as ARIMA(1,0,0) to the detrended data.
...
- The best model I tried had an AIC of -236: ARIMA(0, 0, 1). By running the ar() commands, however, I saw that a lower AIC (-239) was achievable with a structure of ARIMA(2,0,0).
...
```
Both beta parameters converged on values close to 0, so they are probably not important within the model. Our common rho parameter, which came from the repeated measures model, converged on a value close to 1, though there was a lot of autocorrelation for this parameter estimate.
