...
knitr::include_graphics("/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/analysis/images/")
library(knitr)
```

## Overview

This analysis used state-space hierarchical models to explore spatio-temporal trends in the relative abundances of soil microbes. Dozens of microbial groups were modeled at the plot level, with 67 plots located within 5 sites across the United States (Fig 1). Overall, spatial variability eclipsed temporal variability (Fig 2), though many groups displayed sensitivity to environmental conditions (Fig 3). Our predictive ability varied by taxonomic ranking (Fig 4). The majority of our predictive uncertainty came from process error (Fig 5), indicating either that more complexity to capture microbial abundance trends, or that there is inherent stochasticity that statistical models cannot explain.

```{r, echo=FALSE}
include_graphics("Figure1a.png")
include_graphics("Figure1b.png")
include_graphics("Figure1c.png")
```

**Figure 1.** Model fits and forecasts of 3 different microbial groups (a: Acidobacteria, b: Bacteroidetes, c: Ascomycota) at the same plot. Observed points are plotted in black; credible intervals (where we expect the "true mean" to be, with 95% confidence) are in green; and predictive intervals (where we expect observations to be, with 95% confidence) are in blue. As new data is added to the model, the predictions are expected to increase in accuracy.

## Spatiotemporal variability

We hypothesized that spatial variability would eclipse temporal variability. To test this, we included random effects (with a mean of 0) to capture variation specific to plots, sites, and time points. The standard deviation of these random effects was estimated using 1000 Markov Chain Monte Carlo samples from the posterior densities. The these samples were drawn for ten bacterial phyla and ten fungal phyla (resulting in 10000 draws per parameter), and the resulting distributions are visible in Figure 2. Variability among the five study sites was much larger than variability among plots or variability among time points. 

To test whether variability is explainable by environmental conditions (such as temperature, precipitation, and pH), we fit models with and without environmental conditions. As expected, temporal variability became negligible once environmental factors were considered.
...
### Fungi
```{r, echo=F}
include_graphics("Figure3a.png")
```

### Bacteria
```{r, echo=F}
include_graphics("Figure3d.png")
```
...
```{r, echo=F}
include_graphics("Figure5a.png")
include_graphics("Figure5b.png")
```
