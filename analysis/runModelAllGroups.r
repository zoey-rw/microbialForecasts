...
for (j in 1:8){ # j specifies the group (e.g. Acidobacteria)
...
monitor <- c("beta_IC","beta_temp","beta_precip","beta_pH","tau","plot_var","plot_mean","site_effect","plot_effect","time_effect")
...
names(out) <- names(rank.df)[6:10]
saveRDS(out, paste0("/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/data/",names(d)[[k]],"JAGS6-10.rds"))
out.allrank[[k]] <- out
}
