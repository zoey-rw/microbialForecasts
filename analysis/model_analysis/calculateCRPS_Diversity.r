...
hindcast_data_oldsites <- readRDS("/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/data/summary/hindcast_div.rds")
hindcast_data_newsites <- readRDS("/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/data/summary/hindcast_div_newsites.rds")
hindcast_data <- rbind(hindcast_data_oldsites, hindcast_data_newsites, fill=T)
#hindcast_data <- hindcast_data %>% separate(scenario, sep = "\\.", into = c("scenario","plot","datenum"))
hindcast_data$newsite <- ifelse(hindcast_data$newsite, "New site", "Observed site")
...
# Compare means
scored_hindcasts_mean <- scored_hindcasts %>% group_by(group, pretty_group, model_name, newsite) %>% 
...
skill_score <- scored_hindcasts_mean %>% 
	pivot_wider(id_cols = c("pretty_group","model_name"), values_from = "crps_mean", names_from = "newsite") %>% 
