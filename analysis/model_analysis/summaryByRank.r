...
d2 <- readRDS("/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/data/clean/cal_groupAbundances_ITS_2021.rds")
d <- c(d1, d2)
...
for (rank.name in c("phylum_bac","class_bac","family_bac","order_bac","genus_bac",#)) {
	#for (rank.name in c(
		...
		plot_rel_out <- read_in$plot_summary[[2]]
		plot_rel_out <- plot_rel_out[grep("plot_rel", rownames(plot_rel_out)),]
...
saveRDS(list("plot_est" = plot_est_allranks, 
						 ...
						 "gelman" = gelman_list), "/projectnb/talbot-lab-data/zrwerbin/temporal_forecast/data/model_outputs/allrank_summaries.rds")
