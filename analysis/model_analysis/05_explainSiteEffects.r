...
dredge.out = foreach(s=tax_list, .errorhandling = 'remove') %dopar% {
	...
	# t.score=2.039513
	# margin.error <- t.score * new_site_effects$se.fit[1]
	# lower.bound <- new_site_effects$fit[1] - margin.error
	# upper.bound <- new_site_effects$fit[1] + margin.error
	# print(c(lower.bound,upper.bound))
	# 
	# new_site_effects$ci_lo1 = new_site_effects$fit - (new_site_effects$sd.fit * 1.64)
	# new_site_effects$ci_hi2 = new_site_effects$fit + (new_site_effects$sd.fit * 1.64)
	# 
	# head(new_site_effects, 2)
...
saveRDS(list(dredged_predictor_importance, pred_sites), here("data/summary/site_effects_dredged_env_cov.rds"))
saveRDS(unobs_sites, here("data/summary/site_effects_unobserved_env_cov.rds"))
