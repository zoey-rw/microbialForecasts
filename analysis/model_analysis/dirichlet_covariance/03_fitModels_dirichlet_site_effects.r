#!/usr/bin/env Rscript
# Phase 3: Dirichlet model with site effects
# This adds site-level random effects to the temporal dependence model
...
# Set project root
here::i_am("analysis/model_analysis/dirichlet_covariance/03_fitModels_dirichlet_site_effects.r")
project_root <- here()
...
create_directories_safe(
    ...
    c("dirichlet_regression", "dirichlet_regression/site_effects", "dirichlet_regression/temporal", "dirichlet_regression/env_cycl", "dirichlet_regression/env_cov")
)

# Define output directory
model_output_dir <- here("data", "model_outputs", "dirichlet_regression", "site_effects")

cat("==================================================\n")
cat("PHASE 3: Dirichlet with Site Effects\n")
cat("==================================================\n")
...
# Create initial values with hierarchical site effects
inits <- list(
  ...
  sig = 1,
  site_effect_sd = rep(0.5, N.spp),  # Per-species variance for site effects
...
# Create site effects model code
modelCode <- nimbleCode({
  # Dirichlet model with temporal dependence and site effects
  for (i in 1:N.core) {
    y[i, 1:N.spp] ~ ddirch(alpha[plot_num[i], 1:N.spp, timepoint[i]])
  }
  
  # Alpha generation with temporal autocorrelation and hierarchical site effects
  for (s in 1:N.spp) {
    for (p in 1:N.plot) {
      # First timepoint - no temporal dependence
      plot_mu[p, s, 1] ~ dnorm(intercept[s] + site_effect[plot_site_num[p], s], sigma[s])
      alpha[p, s, 1] ~ dgamma(exp(plot_mu[p, s, 1]), 1)
      
      # Subsequent timepoints - AR(1) process with per-taxon rho
      for (t in 2:N.date) {
        plot_mu[p, s, t] ~ dnorm(rho[s] * plot_mu[p, s, t-1] + 
                                 intercept[s] + site_effect[plot_site_num[p], s], 
                                 sigma[s])
  ...
  # Hierarchical site effects - per-species variance
  for (s in 1:N.spp) {
    site_effect_sd[s] ~ dgamma(2, 20)  # Per-species variance for site effects
    for (site in 1:N.site) {
      site_effect[site, s] ~ dnorm(0, site_effect_sd[s])
    }
  }
...
cat("Building Dirichlet model with site effects...\n")
...
mcmcConf <- configureMCMC(cModel, monitors = c("beta", "sigma", "intercept", "rho", "site_effect_sd", "site_effect", "plot_mu"))
...
# Save results
output_file <- here("data", "model_outputs", "dirichlet_regression", "site_effects", "site_effects_test.rds")
saveRDS(list(
  ...
  constants = constants,
  model_data = list(y = y, plot_num = plot_num, timepoint = timepoint, plot_site_num = plot_site_num, keep_taxa = keep_taxa),
  mcmc_settings = list(n_chains = n_chains, n_iter = n_iter, n_burnin = n_burnin)
), output_file)

cat("Results saved to:", output_file, "\n")
cat("=== PHASE 3: SITE EFFECTS TEST SUCCESSFUL ===\n")
cat("This tests site-level random effects in the Dirichlet model!\n")
