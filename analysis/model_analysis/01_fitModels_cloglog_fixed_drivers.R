...
create_directories_safe(
    ...
    c("cloglog_beta_fixed_drivers", "cloglog_beta_fixed_drivers/cycl_only")
)



# Define output directory early to prevent undefined variable errors
model_output_dir <- here("data", "model_outputs", "cloglog_beta_fixed_drivers")
...
create_nimble_model_with_uncertainty <- function(model_name, use_legacy_covariate = TRUE, 
                                                      temporalDriverUncertainty = FALSE, 
                                                      spatialDriverUncertainty = FALSE) {
    cat("Building FINAL Nimble model with FIXED drivers (cloglog version):", model_name, "\n")
        ...
        modelCode <- nimble::nimbleCode({
            ...
            for (p in 1:N.plot) {
                ...
                for (t in (plot_start[p] + 1):N.date) {
                    ...
                    # Linear predictor with FIXED environmental predictors (SAME cloglog link)
                    eta_mean[p, t] <- rho * eta_prev[p, t] +
                        beta[1] * sin_mo[t] + beta[2] * cos_mo[t] +  # Seasonal terms
                        beta[3] * temp[plot_site_num[p], t] + beta[4] * mois[plot_site_num[p], t] +  # Site-level environmental terms (FIXED)
                        beta[5] * pH[p, t] + beta[6] * pC[p, t] +  # Plot-level environmental terms (FIXED)
        ...
        modelCode <- nimble::nimbleCode({
            ...
            for (p in 1:N.plot) {
                ...
                for (t in (plot_start[p] + 1):N.date) {
                    ...
                    # Linear predictor with FIXED environmental predictors (no seasonal terms)
                    eta_mean[p, t] <- rho * eta_prev[p, t] +
                        beta[1] * temp[plot_site_num[p], t] + beta[2] * mois[plot_site_num[p], t] +  # Site-level environmental terms (FIXED)
                        beta[3] * pH[p, t] + beta[4] * pC[p, t] +  # Plot-level environmental terms (FIXED)
...
run_scenarios_fixed <- function(j, chain_no) {
    ...
    tryCatch({
        ...
        constants$temporalDriverUncertainty <- FALSE  # Fixed drivers - no temporal uncertainty
        constants$spatialDriverUncertainty <- FALSE   # Fixed drivers - no spatial uncertainty
        ...
        # Skip driver uncertainty data for FIXED driver models
        if (FALSE && model_name %in% c("env_cycl", "env_cov")) {
        ...
        # Create model with FIXED drivers (cloglog version)
        cat("Building Nimble model with FIXED drivers (cloglog version)...\n")
        modelCode <- create_nimble_model_with_uncertainty(model_name, use_legacy_covariate, 
                                                         temporalDriverUncertainty = FALSE, 
                                                         spatialDriverUncertainty = FALSE)
        
        # Create initial values with FIXED drivers
        cat("Creating initial values with FIXED drivers...\n")
        ...
        # Skip driver uncertainty parameters for FIXED driver models
        # if (model_name %in% c("env_cycl", "env_cov")) {
        #     monitored_params <- c(monitored_params, "temp_est", "mois_est", "pH_est", "pC_est")
        # }
        ...
        mcmcConf$removeSamplers(c("precision", "rho", "beta", "site_effect_sd", "site_effect", "intercept", "legacy_effect"))
        ...
        burnin <- 2000  # Production: Adequate burn-in for convergence
        thin <- 1
        iter_per_chunk <- 1000  # Production: Larger chunks for efficiency
        init_iter <- 2000  # Production: Adequate initial iterations
        min_eff_size_perchain <- 100  # Production: Target ESS for reliable estimates
        max_loops <- 5  # Production: Allow more loops for convergence
        max_save_size <- 50000
        ...
        chain_output <- list(
            ...
            metadata = list(
                ...
                has_driver_uncertainty = FALSE,  # Flag to identify FIXED driver models
                scenario = scenario,
                min.date = min.date,
                max.date = max.date,
                ...
                model_structure = "cloglog_beta_regression_with_fixed_drivers"
            )
        )
        ...
        cat("=== Model fitting completed with FIXED DRIVERS (cloglog version) ===\n")
        cat("  - STABLE: Beta regression with FIXED environmental predictors\n")
        cat("  - FIXED DRIVERS: Temperature, moisture, pH, and pC treated as observed values\n")
        cat("  - SAME CLOGLOG LINK: cloglog transformation (1 - exp(-exp(eta))) for (0,1) bounds\n")
        ...
        cat("  - PURPOSE: Isolate pure effect of driver uncertainty vs link function differences\n")
        ...
        error_dir <- here("data", "model_outputs", "cloglog_beta_fixed_drivers", model_name)
        ...
        if (result$status == "SUCCESS") {
            ...
            for (base_dir in possible_bases) {
                if (!is.null(base_dir) && base_dir != "" && base_dir != "NULL") {
                    test_dir <- file.path(base_dir, "cloglog_beta_fixed_drivers", valid_models$model_name[model_idx])
                    tryCatch({
            ...
            if (is.null(model_output_dir)) {
                ...
                fallback_output_dir <- here("data", "model_outputs", "cloglog_beta_fixed_drivers", valid_models$model_name[model_idx])
            ...
            if ("metadata" %in% names(result) && !is.null(result$metadata)) {
                ...
                metadata <- list(
                    ...
                    model_structure = "cloglog_beta_regression_with_fixed_drivers"  # Model structure identifier
                )
            }
