# Determine important predictors of site effects (random effects) for truncNorm models
...
# Read in site effect estimates from truncNorm models; split into calibration timeperiod and full dataset
site_effects_all <- readRDS(here("data/summary/truncated_normal_site_effects.rds")) %>% 
  filter(nchar(siteID) > 3) #remove numeric siteIDs
...
df_observed <- site_effects_calibration %>%
  select(siteID,model_id,Median,rank,rank_only,group,taxon,pretty_group) %>%
...
for(s in model_id_list) {
  ...
  tryCatch({
    ...
    unobserved_file <- here("data/summary", paste0("site_effects_unobserved_", model_type, ".rds"))
...
# Save truncNorm-specific outputs
saveRDS(list(dredged_predictor_importance,pred_sites,pred_sites_plsr,plsr_model_importance), here("data/summary/site_effects_dredged_truncNorm.rds"))
saveRDS(list(unobs_sites,unobs_sites_plsr), here("data/summary/site_effects_unobserved_truncNorm.rds"))

cat("✅ truncNorm site effects prediction completed successfully!\n")
cat("Output files saved:\n")
cat("  - site_effects_dredged_truncNorm.rds\n")
cat("  - site_effects_unobserved_truncNorm.rds\n")

# Test reading the output
read_in = readRDS(here("data/summary/site_effects_dredged_truncNorm.rds"))
