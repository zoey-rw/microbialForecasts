...
# Set project root
here::i_am("analysis/model_analysis/01_fitModels_stable.R")
project_root <- here()
...
params <- params_in %>% ungroup %>% filter(
  ...
  rank.name %in% c("phylum_fun", "class_fun", "order_fun", "family_fun")
)
...
params <- params %>%
  sample_n(size = min(10, n()), replace = FALSE) %>%  # Run up to 10 models for HPC
  ungroup()
