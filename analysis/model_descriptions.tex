...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../data/figures/fig_compare_CLR_betareg.png}
...
\rho &\sim \mathrm{Beta}(1,\, 1) & &\text{(equivalent to Uniform(0,1))} \notag\\
\alpha_0 &\sim \mathcal{N}(0,\, 10^2) \notag\\
\delta &\sim \mathcal{N}(0,\, 10^2) & &\text{(legacy effect)} \notag\\
...
Process layer & AR(1) on $\eta$ & AR(1) on $\eta$ & Beta on $\mu$ & Gamma on $\alpha$ \\
Multivariate & No & No & No & Yes ($S$ species) \\
Driver uncertainty & Yes & Yes & No & No \\
$\rho$ prior & Beta$(1,1)$ & Beta$(1,1)$ & $\mathcal{N}(0,1)$ & Unif$(-0.99, 0.99)$ \\
$\beta$ prior SD & 10 & 10 & 0.5 & MVN (identity) \\
\bottomrule
\end{tabular}
...
The code below shows the full env\_cycl model with environmental covariates, seasonal predictors, and optional driver uncertainty.
