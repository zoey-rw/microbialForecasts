...
fg_rsq <- scores_list$scoring_metrics_long %>%
	filter(model_id %in% scores_list$converged_list) %>%
	filter(metric %in% c("RSQ.1","RSQ","RMSE.norm")) %>%
	mutate(score = ifelse(score < 0, 0, score)) %>%
	filter(model_name == "env_cycl" & pretty_name == "functional") %>%
	distinct()  %>%
...
# in glo
fg_rsq$pretty_fg_names <- recode(fg_rsq$taxon, !!!pretty_names)
...
ggplot(fg_rsq %>%
	...
	geom_jitter(aes(color=pretty_group), size=4, #width = .3, height=0, 
							alpha=.5, 
							position=pos) +
	xlab(NULL) + labs(fill='') +
	...
	ylab("Forecast RSQ") + labs(color=NULL) +
	geom_label_repel(aes(label=pretty_fg_names), size=4, max.overlaps = 20, position=pos) + 
	ggpubr::stat_pvalue_manual(stat_pvalue_fg_source, 
														 label = "p.adj.signif") #, bracket.nudge.y = -.4, size=4, hide.ns = T)

# Looking at functional group category
ggplot(fg_rsq %>%  filter(metric %in% "RSQ" & site_prediction == "New time (observed site)"), aes(x = fg_category, y = as.numeric(score)))  +
	geom_boxplot() +
