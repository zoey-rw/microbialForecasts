...
df_cal_fg_tax <- rbind(df_tax, df_fg) %>% filter(!beta %in% c("sin","cos")) %>%
	rbind(seas_vals, fill=T) %>% filter(model_name=="all_covariates")
df_cyclical <- rbind(df_tax, df_fg) %>% filter(!beta %in% c("sin","cos")) %>%
	rbind(seas_vals, fill=T) %>% filter(model_name != "all_covariates")
...
ranks_beta_plot <- ranks_beta_plot  +
	...
	geom_text(data = tukey_list_tax_fg,
																							 aes(x = only_rank, y = tot, color =pretty_group,
