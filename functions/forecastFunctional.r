...
#	params <- model_out %>% pivot_wider(id_cols = taxon, names_from = "rowname", values_from = c(Mean, SD))
	...
	to_loop <- 1:nrow(val_plot_key)
	 if(test) to_loop <- to_loop[c(1,5,10,20,30,31,32,33,40,50)]
		...
		if (!is_new_site) {
			...
			plot_est <- model_plot_est %>% 
				filter(taxon==!!taxon_name & 
							 	...
							 	time_period == !!time_period &
							 	model_name == !!model_name) %>% 
				select(-c(date_num,plot_num, site_num,taxon)) %>% rename(taxon = name)
			# 
...
#		sigma_samp <- truncnorm::rtruncnorm(Nmc, mean = sigma$Mean, sd = sigma$SD, a = 0)
		...
		beta_samp <- matrix(rep(beta$Mean, Nmc), ncol = N.beta, byrow = T)
		rho_samp <- rep(rho$Mean, Nmc)
		site_effect_samp <- rep(site_effect$Mean, Nmc)
		...
		x <- mean(as.numeric(truth.plot.long[truth.plot.long$siteID==siteID,]$truth), na.rm=T)
		...
		if (!is_new_site) {
			plot_est <- model_plot_est %>% 
				filter(taxon==!!taxon_name & plotID == !!plotID & time_period == !!time_period & model_name == !!model_name) %>% 
