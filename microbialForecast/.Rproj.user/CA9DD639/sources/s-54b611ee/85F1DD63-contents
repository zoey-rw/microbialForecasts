...
div_hindcast_data <- readRDS("./data/summary/hindcast_div.rds") %>%
	mutate(fcast_type = "Diversity", rank = paste0("diversity_", group),
				 taxon = rank)
fg_hindcast_data <- readRDS("./data/summary/hindcast_fg_2015-11_2018-01.rds") %>%
...
hindcast_data$group <- ifelse(grepl("_bac", hindcast_data$rank, fixed = T), "16S", "ITS")
...
ggplot(skill_score_rank %>% filter(model_name == "all_covariates"),
