...
#params <- params[(params$scenario == "2 year current methods"  & params$model_name == "all_covariates"),]
...
run_scenarios <- function(j, chain) {
	print(params[j,])
	out <- run_MCMC_dirichlet(k = params$group[[j]],
											# iter_per_chunk = 50000,
											# init_iter = 150000,
											 iter_per_chunk = 5000,
											 init_iter = 10000, burnin = 5000,
											 #burnin = 100000,
											thin = 15, chain_no = chain,
