...
prepDivData <- function(rank.df,
	...
	dat <- dat %>% arrange(siteID, plotID, dateID) %>% mutate(plot_date = paste0(plotID, "_", dateID))
	...
	dat <- dat %>% mutate(site_hor = paste0(siteID, horizon)) %>%
	...
	filt_predictor_data <- lapply(predictor_data, FUN = filter_date_site,
																keep_sites = keep_sites,
																keep_plots = keep_plots,
																min.date,
																max.date, max.predictor.date)
