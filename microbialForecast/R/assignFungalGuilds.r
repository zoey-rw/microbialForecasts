...
#' @title assign_fungal_guilds
#' @description Assign fungal functional guilds to a taxonomy table by combining three
#'   reference sources: the FUNGuild database (higher-level classifications), the FunFun /
#'   fungaltraits database (genus/species level), and the FungalTraits database (Polme et al.
#'   2020). Guild calls are collapsed with priority fungaltraits > funfun > funguild, then
#'   broken out into binary columns for the modeled guilds (endophyte, plant_pathogen,
#'   animal_pathogen, ectomycorrhizal, lichenized, wood/soil/litter_saprotroph, saprotroph).
#' @param tax_table A data.frame taxonomy table with columns down to genus/species
#'   (column names are coerced to lower case).
#' @param url URL of the FUNGuild database (passed through for reference; the assignment uses
#'   \code{FUNGuildR::funguild_assign}).
#' @param fungaltraits_path Path to the FungalTraits reference CSV (Polme et al. 2020).
#'   Defaults to \code{data/reference_data/FungalTraitsDB.csv} under the project root.
#' @param n.cores Number of cores for the parallel guild lookups. Defaults to all detected cores.
#' @return The input taxonomy table with binary guild columns appended.
#' @importFrom foreach foreach %dopar%
#' @export
assign_fungal_guilds <- function(tax_table, url = "http://www.stbates.org/funguild_db.php", fungaltraits_path = here::here("data/reference_data/FungalTraitsDB.csv"), n.cores = NA){
